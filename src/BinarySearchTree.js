class BinarySearchTree {
  ...
  Insert(key,value) {
      ...
      /* If the tree already exists, then start at the root, compare it to the key that you want to insert.
      If the new key is less than the node's key, then the new node needs to live in the left-hand branch */
          ...
          /* If the node has an existing left child, then you recursively call the `insert()` method
  ...
  find(key) {
    ...
    /* If the item that you are looking for is greater than the root, then follow the right child.
       If there is an existing right child, then recursively check its left and/or right child
       until you find the item. */
